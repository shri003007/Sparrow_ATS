...
interface GamesArenaCandidatesTableProps {
...
export function GamesArenaCandidatesTable({ 
  candidates,
  ...
  currentStatusById,
  onCandidateUpdated
}: GamesArenaCandidatesTableProps) {
  const fontFamily = "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif"
    ...
    <>
      ...
      {selectedCandidate && (
        <CandidateEvaluationPanel
          ...
          sparrowRoundId=""
          currentRoundName={roundInfo?.round_name || 'Games Arena Round'}
        />
      )}
    </>
  )
